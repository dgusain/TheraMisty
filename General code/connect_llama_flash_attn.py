...
class Config:
    ...
    expressions = {"excite","sad","hug","think","listen"}
    char_rate = 17
...
class MistyAssistant:
    def __init__(self):
        ...
        self.messages: List[Dict[str, str]] = [
            {"role": "system", "content": "You are Misty, a speech language pathologist robot designed to practice minimal pair therapy on users. Perform word exercises with the user and correct their pronounciation. Converse with the users in a friendly tone."}
        ]
    ...
    def load_model(self):
        try:
            ...
            logger.info("Model and tokenizer loaded successfully with Flash Attention.")
    ...
    def run(self):
        try:
            ...
            while True:
                ...
                # pass the speech in chunks so that misty can express in between. 
                #response.split(exp)

                #logger.info("Length of response: ", len(response))
                duration_speaking = len(response)/Config.char_rate # average TTS speaking time: 18 characters per second
